...
def find_closest_state(tweet, state_centers):
    ...
    """
    "*** YOUR CODE HERE ***"
    trier = tweet_location(tweet)
    test = float("inf") 
    ja = 0
    for key, value in state_centers.items():
        if geo_distance(trier, value) < test:
            test = geo_distance(trier, value)
            ja = key
    return ja
...
def average_sentiments(tweets_by_state):
    ...
    """for key, value in tweets_by_state.items():  
        if analyze_tweet_sentiment(value) != False:
            if key in D:
                counter += 1
                D[key].append(value)
            else: 
    ...
    for state, tweets in tweets_by_state.items():
        ...
        for tweet in tweets:
            ...
            if valid_tweets > 0:
                averaged_state_sentiments[state] = total_sentiment / valid_tweets
            ...
            if elem == ",":
                ...
                if counter == 3:
                    ...
                    if T(M+2) != ",":
                        ...
                        C2 = int(C2)
                        C1 = C1 + C2
                    tweets_by_hour[C1].append(tweet)"""
    
    for tweet in tweets:
        tweet_time_obj = tweet_time(tweet)

        tweet_hour = tweet_time_obj.hour
